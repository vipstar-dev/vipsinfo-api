...
class TransactionService extends Service {
  async getTransaction(id) {
    ...
    const {Address: RawAddress} = this.app.vipsinfo.lib
  ...
  async getRawTransaction(id) {
    ...
    const {Transaction: RawTransaction, Input, Output, OutputScript} = this.app.vipsinfo.lib
  ...
  async sendRawTransaction(data) {
    let client = new this.app.vipsinfo.rpc(this.app.config.vipsinfo.rpc)
  ...
  transformInput(input, index, transaction, {brief}) {
    const {InputScript, OutputScript} = this.app.vipsinfo.lib
  ...
  transformOutput(output, index, {brief}) {
    const {OutputScript} = this.app.vipsinfo.lib
  ...
  async transformQRC20Transfers(outputs) {
    const TransferABI = this.app.vipsinfo.lib.Solidity.qrc20ABIs.find(abi => abi.name === 'Transfer')
    let result = []
  ...
  async transformQRC20UnconfirmedTransfers(outputs) {
    const {OutputScript, Solidity} = this.app.vipsinfo.lib
  ...
  async getContractTransaction(receiptId) {
    const {Address: RawAddress, OutputScript} = this.app.vipsinfo.lib
    const {
